"""
Extended PPO Training - Original CartPoleEnv
10M steps - hoping for better convergence
"""
import os
import time
import numpy as np
...
def main():
    num_cpu = 4
    total_timesteps = int(1e7)  # 10M steps

    timestamp = time.strftime("%Y%m%d_%H%M")
    log_dir = f"./tb_logs/PPO_CartPole_Extended_{timestamp}_{total_timesteps}/"
    model_dir = f"./saved_models/PPO_CartPole_Extended_{timestamp}_{total_timesteps}/"
    os.makedirs(log_dir, exist_ok=True)
    os.makedirs(model_dir, exist_ok=True)

    print("[Training Config Extended PPO]")
    print("  Environment: CartPoleEnv (original)")
    print("  Total timesteps: 10,000,000")
    print("  Network: default (smaller)")
    print("")
    ...
    print("[Training] Starting extended PPO training (10M steps)...")
