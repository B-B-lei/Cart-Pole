"""
Cart-Pole Balance Environment v5
- Focus on making reward differences larger and clearer
- 45 degree termination

Changes from v4:
1. Much larger angle reward scaling (factor of 50 instead of 5)
2. Added position penalty to encourage centering
3. Larger velocity penalty for stability
"""
...
class CartPoleEnvV5(MujocoEnv, utils.EzPickle):
    ...
    def step(self, action):
        ...
        # v5 reward: LARGER differences to help learning
        # Survival bonus
        r_survival = 4.0

        # Angle reward: LARGE exponential (50 instead of 5)
        # This creates big differences between good/bad states
        k_angle = 50.0
        r_angle = np.exp(-k_angle * (theta_1**2 + theta_2**2))

        # Velocity penalty: larger to discourage wobbling
        k_vel = 0.05
        r_vel = -k_vel * (theta_dot_1**2 + theta_dot_2**2 + 0.5 * chassis_vx**2)

        # Position penalty: small penalty for being away from center
        k_pos = 0.01
        r_pos = -k_pos * chassis_vx**2

        # Combined reward
        reward = r_survival + r_angle + r_vel + r_pos
