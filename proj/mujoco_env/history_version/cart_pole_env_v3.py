"""
Cart-Pole Balance Environment v3
- Termination: 45 degrees
- Reward: Positive reward for small angles, scaled survival bonus
- Observation: 8-dim (theta_L, theta_R, theta_1, theta_2, theta_L_dot, theta_R_dot, theta_dot_1, theta_dot_2)
- Action: 1-dim continuous (dual-wheel torque)
"""
...
class CartPoleEnvV3(MujocoEnv, utils.EzPickle):
    ...
    def step(self, action):
        ...
        # v3 reward: Positive reward design
        # Base survival reward
        r_survival = 2.0

        # Angle reward: exponential reward for staying close to vertical
        # exp(-k*theta^2) gives value in (0,1], max at theta=0
        k_angle = 10.0
        r_angle = np.exp(-k_angle * (theta_1**2 + theta_2**2))

        # Velocity penalty: small penalty to discourage excessive movement
        k_vel = 0.001
        r_vel = -k_vel * (theta_dot_1**2 + theta_dot_2**2 + chassis_vx**2)
