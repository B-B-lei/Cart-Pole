"""
PPO Training Script v8
- Fixed sensor extraction bug
- Using CartPoleEnvV8
- 3M steps with 256x256 network
"""
import os
import time
import numpy as np
...
def main():
    ...
    log_dir = f"./tb_logs/PPO_CartPole_v8_{timestamp}_{total_timesteps}/"
    model_dir = f"./saved_models/PPO_CartPole_v8_{timestamp}_{total_timesteps}/"
    os.makedirs(log_dir, exist_ok=True)
    os.makedirs(model_dir, exist_ok=True)

    print("[Training Config v8]")
    print("  Environment: CartPoleEnvV8 (FIXED sensor extraction)")
    print("  Network: 256x256")
    print("  Total timesteps: 3,000,000")
    print("")

    parallel_train_env = SubprocVecEnv([make_env(i, log_dir) for i in range(num_cpu)])

    eval_env = CartPoleEnvV8()
